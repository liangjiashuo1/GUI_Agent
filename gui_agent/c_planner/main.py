...
2. 如果需要搜索：点击搜索框 → TYPE 关键词 → CLICK 搜索结果中匹配的剧集
3. **重要** 搜索后进入的是剧集详情页（有"立即播放"按钮或者"继续播放"按钮和"选集""简介"等标签），此时绝对不能直接找评论区！必须先点击"立即播放"按钮或者"继续播放"按钮进入全屏播放器，评论区入口只在播放器内存在
4. **进入播放器后**，找到文字为"讨论"的标签（Tab），这是永久的评论区入口。不要点击临时弹出的评论浮窗（会自动消失）
5. 评论区入口就是文字为"讨论"的Tab，按OCR列表中的"讨论"文字找到它并点击，不要依赖屏幕位置猜测
6. 如果目标元素不在当前屏幕且页面可滚动 → SCROLL
7. 如果输入框已激活且需要输入文字 → TYPE
8. 如果要发评论：进入讨论区后 → 输入框固定在屏幕最下方（y坐标通常>900）→ 点击输入框 → TYPE 评论内容 → CLICK 发送按钮
9. 历史中已经失败过的操作不要重复
10. 确认所有子任务（打开App、搜索、进入播放器、输入文字、提交）都完成后才能 COMPLETE
11. target_id 必须从下方元素列表中选取，严禁编造"""
...
class PlannerModule:
    ...
    def plan_next_action(
        ...
        # 还没打开目标 App（若当前已在目标 App 中则不重复打开）
        target_app = self._guess_app_name(input_data.instruction)
        already_in_target = (
            target_app
            and perception.app_name == target_app
            and perception.page_type not in ("unknown", "home")
        )
        if (perception.app_name
                and not self._has_opened_app(memory)
                and not already_in_target):
            return PlannerDecision(
                action=ACTION_OPEN,
    ...
    def _enforce_hard_constraints(
        ...
        # 3. 搜索页 + 搜索词未输入 → 强制 TYPE（不等键盘，TYPE 本身会触发键盘弹出）
        if perception.page_type == "search" and search_keyword and not has_typed_search:
        ...
        # 3.5 评论区已进入（有"最热""最新"标签）+ 评论未输入 → 直接点底部输入框
        if required_text and not has_typed_required:
            all_text = " ".join(e.text or "" for e in perception.elements)
            in_comment_section = any(kw in all_text for kw in ["最热", "最新"])
            if in_comment_section:
                bottom_point = self._find_bottom_input(perception.elements, perception)
                if bottom_point:
                    logger.info("已进入评论区(检测到最热/最新)，点击底部输入框: %s", bottom_point)
                    return PlannerDecision(
                        action=ACTION_CLICK,
                        parameters={"point": bottom_point},
                        thought="评论区已打开，点击底部输入框准备输入评论",
                        confidence=0.95, is_terminal=False,
                    )
    ...
    @staticmethod
    def _find_bottom_input(elements: List[UIElement], perception=None):
        """找评论区底部输入框：y>850 的元素中取最下最宽的，优先input/button角色。"""
        # 优先找底部input/button角色
        candidates = [e for e in elements
                      if e.bbox and len(e.bbox) == 4 and e.bbox[1] > 850
                      and e.role in ("input", "button")]
        if not candidates:
            # 放宽：底部任意短文字元素
            candidates = [e for e in elements
                          if e.bbox and len(e.bbox) == 4 and e.bbox[1] > 850
                          and e.text and len(e.text) <= 20]
        if not candidates:
            # 再放宽：y>750
            candidates = [e for e in elements
                          if e.bbox and len(e.bbox) == 4 and e.bbox[1] > 750]
        if not candidates:
            return None
        best = max(candidates, key=lambda e: (e.bbox[1], e.bbox[2] - e.bbox[0]))
        return [(best.bbox[0] + best.bbox[2]) // 2, (best.bbox[1] + best.bbox[3]) // 2]
